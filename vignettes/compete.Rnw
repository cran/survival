...
The first is that it attempts to estimate the expected rate of
...
@ 
The FG model states that males have a lower rate of \emph{observed} PCM, 
...
``females have a 1.2 fold higher chance of PCM, after adjustment for age 
and serum m-spike''.
