...
concordancefit <- function(y, x, strata, weights, ymin=NULL, ymax=NULL, 
 ...
 survival object, or an orderable factor")
    ...
    docount <- function(y, risk, wts, timeopt= 'n', timefix) {
        ...
        if (nevent==1) timeopt <- 'n'
        
        if (timeopt == 'n') timewt <- rep(1.0, n)
        else {
            ...
            if (timeopt %in% c("S/G", "n/G", "n/G2")) {
                ...
                if (timefix) temp <- aeqSurv(y)
                else temp <- y
                temp[, ny] <- 1- temp[, ny] # switch event/censor
                delta <- min(diff(unique(y[, -ny])))/2
...
pairs, respectively,
and $m= C+D+ T$ will be the total number of comparable pairs.
...
of 16 in the tree.
The concordant pairs are those with a risk score $>16$, i.e., both $\hat y=x$
and $y$ are larger, discordant are $<16$, and we have no ties.
...
The following two routines arrange values into such a tree.  The first is
more elegant and the second is faster for large n.
...
# this one is faster for large n
# Say n=13.  The lowest level of the tree has elements 1, 3, 5, 7, 9, 11
#      mapped to positions 8:13
#  the next level has 2, 6, 10, 13 =  half the remaining, positions 4:7
#  the next level has 4, 13       =  half the remaining, positions 2:3
#  the top has the 7th largest mapped to position 1
# positions are numbered from 0 since they will be used in C.
btree <- function(n) {
    position <- integer(n)  #will be overwritten
    yet.to.do <- 1:n
    depth <- floor(logb(n,2))
    start <- as.integer(2^depth)
    lastrow.length <- 1+n-start
    indx <- seq(1L, by=2L, length= lastrow.length)
    position[yet.to.do[indx]] <- start + 0:(length(indx)-1L)
    yet.to.do <- yet.to.do[-indx]
    
    while (start >1) {
        start <- as.integer(start/2)
        indx <- seq(1L, by=2L, length=start)
        position[yet.to.do[indx]] <- start + 0:(start-1L)
        yet.to.do <- yet.to.do[-indx]
    }
    position
}
