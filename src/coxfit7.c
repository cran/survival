...
static double coxfit7_iter(int nvar, int nused, int method, double *beta);

SEXP coxfit7(SEXP maxiter2,  SEXP time2,   SEXP status2, 
    ...
    *iter =0; 
    strata[nused-1] =1;
    loglik[0] = coxfit7_iter(nvar, nused, method, beta);
    loglik[1] = loglik[0];
	...
	*/
	...
	newlk = coxfit7_iter(nvar, nused, method, newbeta);
	*flag = cholesky2(imat, nvar, toler);
    ...
    */
    if (maxiter > 1) 
	loglik[1] = coxfit7_iter(nvar, nused, method, beta);
    chinv2(imat, nvar);
    for (i=0; i<nvar; i++) {
